...
export default function MyCart() {
  ...
  const getDataFromDB = async () => {
    ...
    if (items) {
      ...
      // sepetteki ürüne miktar ekleyebilmek için dönüp produtcData dizisine veriyi ekledik
      Items.forEach(data => {
        if (items.includes(data.id)) {
          data.quantity = 1; // eklenilen id li ürünlerin miktarlarını 1 yap
          productData.push(data); // productData dizisine ekle
        }
      });
      // product state i güncellendi
